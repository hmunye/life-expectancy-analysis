...
Recursive Feature Elimination of the SVR model
...
RFE is not working, the code runs forever
...
**\*\*ADD TABLE HERE COMPARING THE DIFFERENT MODELS R\^2 AND RMSE** ONCE YOU FIGURE OUT THE SVR

------------------------------------------------------------------------

### 5.2 Binaray Analysis: Can we predict Economic Development from Life Expectancy
...
train_data_Log <- df[training.samples, ]
test_data_Log <- df[-training.samples, ]
```
...
LE_LOG <- glm(test_data_Log$Economy_status_Developed ~ Life_expectancy, data = test_data_Log, family = binomial)
```

Next is to make the predictions

```{r}
predictions <- predict(LE_LOG, type = "response")

predicted_Values_Log <- ifelse(predictions > 0.5, "Developed", "Developing")

ConfusionMatrix_Log <- table(predicted_Values_Log, )
```
