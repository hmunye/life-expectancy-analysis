...
partiton_index <- createDataPartition(df$Economy_status_Developed, p = 0.7, list = FALSE)

train_data_log <- df[partiton_index,]
validation_data_log <- df[-partiton_index,]
```
...
Please note that this "Best" model is simply the one with the highest sum of sensitivity and specificity.

#### 5.2.2 XG Boost Model

Convert data to work with XG Boost

```{r}
# Convert character columns to factors
train_data_log$Country <- as.factor(train_data_log$Country)
train_data_log$Region <- as.factor(train_data_log$Region)

# Convert factors to dummy variables
train_data_numeric <- model.matrix(~ . - 1 - Economy_status_Developing, data = train_data_log)
validation_data_numeric <- model.matrix(~ . - 1 - Economy_status_Developing, data = validation_data_log)

train_matrix <- xgb.DMatrix(data = train_data_numeric, label = train_data_log$Economy_status_Developed)
validation_matrix <- xgb.DMatrix(data = validation_data_numeric, label = validation_data_log$Economy_status_Developed)
```
...
xgb_model <- xgb.train(params = params, 
                       data = train_matrix, 
                       nrounds = 100,
                       watchlist = list(eval = validation_matrix, train = train_matrix),
                       print.every.n = 10,
...
```{r}
validation_predictions <- predict(xgb_model, validation_matrix)
binary_validation_predictions <- ifelse(validation_predictions > 0.5, 1, 0)  # Using default threshold of 0.5 initially

confusion_matrix_xgb <- confusionMatrix(factor(binary_validation_predictions), factor(validation_data_log$Economy_status_Developed))
...
partiton_index <- createDataPartition(df$Economy_status_Developed, p = 0.7, list = FALSE)
train_data_DT <- df[partiton_index,]
validation_data_DT <- df[-partiton_index,]
...
| Country       | Mortality | BMI   | Alcohol | Hepatitis B | Measles | Polio  |
|---------------|-----------|-------|---------|-------------|---------|--------|
| Tunisia       | 120       | 25.69 | 1.29    | 96.75%      | 97.81%  | 97.69% |
| Algeria       | 150       | 24.87 | 0.40    | 88.31%      | 93.25%  | 91.75% |
| Zimbabwe      | 600       | 23.54 | 2.90    | 79.56%      | 64.00%  | 79.60% |
| Lesotho       | 690       | 24.49 | 2.62    | 86.63%      | 70.44%  | 87.75% |
| United States | 116       | 28.27 | 8.60    | 91.5%       | 86.00%  | 92.25% |
