...
This model has a very high R-Squared, but we need to be wary of over-fitting. Lets remove the variables that are least significant.

##### 5.1.3 Pruning the Model 

selecting variables, Note that Infant mortality was ommitted due to its high co-varaince with Under Five Deaths.

```{r}
# Selecting specific variables for the regression model
selected_variables <- c("Under_five_deaths", "Adult_mortality", "Alcohol_consumption", "Hepatitis_B", "BMI", "Incidents_HIV", "GDP_per_capita", "Schooling")

# Fit multiple regression model using selected variables
LE_MR2 <- lm(Life_expectancy ~ ., data = train.data[, c("Life_expectancy", selected_variables)])
