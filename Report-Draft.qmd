...
-   [**5 Models**]
    ...
    -   [5.1 Multiple Regression of Life Expectancy]
        ...
        -   [5.1.3 Pruning the Model]
            ...
            -   [5.1.3.2 Removing variables one at a time]
...
What are the predicting variables affecting life expectancy? - multiple regression -(Use 3 different , MR, Forest, SVR, XG Boost, ANN (Not deep) ) - How does Infant and Adult mortality rates affect life expectancy? - Does Life Expectancy have a positive or negative correlation with - BMI, - disease, - drinking alcohol, - immunizations, etc.? - What is the impact of schooling on the lifespan of humans? - Can we predict lifespan based off of schooling? - Do densely populated countries tend to have lower life expectancy? - What is the impact of Immunization coverage on life Expectancy? - Will there be an increase in life expectancy in the next 10 years? - Will the rate of infant deaths go down over the next 5 years? - Is there a correlation between economic status and Life expectancy? - Can we predict countries economic status based off Life expectancy? - Do certain regions have higher vaccination rates? SVN DECESION TREE/FOREST XG BOOST) - (Make one for Binary CLASSIFICATION) Logistic Regression Model
...
Positive: BMI, Polio, Diphteria, Schooling Alcohol:
...
LE_MR <- lm(Life_expectancy ~ . - Country - Region -Year -Economy_status_Developed -Economy_status_Developing, data = train.data)

#print summary of the model
summary(LE_MR)
```

This answers one of our questions, of what variables most influence Life Expectancy

This model has a very high R-Squared, but we need to be wary of over-fitting as well as other potential issues sush as multicollinearity values.
...
There are several variables that have high multicollinearity. From the co-variation analysis done in 4.3 we know that Infant deaths and under five deaths are very closly related, as well as the two thinness variables, and Polio and Diphteria are related.
...
As neither polio nor diphteria are significant to the model we will be dropping both.
...
```{r}
#make the multiple Regression model for Life Expectancy 
LE_MR2 <- lm(Life_expectancy ~ . - Country - Region -Year -Economy_status_Developed -Economy_status_Developing -Polio -Diphtheria -Infant_deaths -Thinness_five_nine_years, data = train.data)
...
```{r}
#make the multiple Regression model for Life Expectancy 
LE_MR3 <- lm(Life_expectancy ~ . - Country - Region -Year -Economy_status_Developed -Economy_status_Developing -Polio -Diphtheria -Infant_deaths -Thinness_five_nine_years -Measles, data = train.data)
...
```{r}
#make the multiple Regression model for Life Expectancy 
LE_MR4 <- lm(Life_expectancy ~ . - Country - Region -Year -Economy_status_Developed -Economy_status_Developing -Polio -Diphtheria -Infant_deaths -Thinness_five_nine_years -Measles -Population_mln, data = train.data)
...
```{r}
#make the multiple Regression model for Life Expectancy 
LE_MR5 <- lm(Life_expectancy ~ . - Country - Region -Year -Economy_status_Developed -Economy_status_Developing -Polio -Diphtheria -Infant_deaths -Thinness_five_nine_years -Measles -Population_mln -Hepatitis_B, data = train.data)
...
```
As deaths under five go up, life expectancy goes down
As adult mortality goes up, life expectancy goes down
As alcohol consumption goes up, so does life expectancy
As BMI goes up, so does life expectancy
As incidents of HIV goes up, life expectancy goes down
As GDP goes up, so does life expectancy
As thinness goes up in prevalence, life expectancy goes down
As schooling goes up, so does life expectancy
...
```{r}
# Prediction #
output <- compute(nn, test_data_nn[, !(names(test_data_nn) %in% c("Life_expectancy"))])

# Extracting predicted values
predicted_values <- output$net.result

# Combining predicted values with actual values
results <- data.frame(Actual = test_data_nn$Life_expectancy, Predicted = predicted_values)


rounded_results <- sapply(results, round, digits = 0)

# Display the results
head(rounded_results)
```

Confusion Matrix

```{r}
# Ensure that unique levels in actual and prediction vectors match
all_levels <- unique(c(actual, prediction))

# Convert actual and prediction vectors to factors with the same levels
...
library(caret)
confusionMatrix(actual_factor, prediction_factor)
```
