...
-   What are the predicting variables affecting life expectancy? - multiple regression -(Use 3 different , MR, Forest, SVR, XG Boost, ANN (Not deep) )
...
-   \- (Make one for Binary CLASSIFICATION) Logistic Regression Model
...
```{r}
LE_SVR <- svm(train.data.svm$Life_expectancy ~ . , train.data.svm)
...
```{r}
predicted_Values_SVR <- predict(LE_SVR , newdata = test.data.svm)
```

Evaluate Model

```{r}
actual_values_SVR <- test.data.svm$Life_expectancy 
mae <- mean(abs(predicted_Values_SVR - actual_values_SVR))
mse <- mean((predicted_Values_SVR - actual_values_SVR)^2)
rmse <- sqrt(mse)
r_squared <- cor(predicted_Values_SVR, actual_values_SVR)^2
...
\*I have NO CLUE how to figure out which variables are significant? Ask Prof Q-

\*\* This is 100% overfitted, but again, which variable is the least significant??

------------------------------------------------------------------------

### 5.4 Random Forest 

Why is this one giving me such a hard time...
